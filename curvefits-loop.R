...
shortlist <- sample((clim.tab.fill[!is.na(clim.tab.fill$Station_ID) & !clim.tab.fill$Station_ID %in% '' & !is.na(clim.tab.fill$p.sum),]$Station_ID),
                    500)
for(ii in 1:length(shortlist)){#ii=1
station <- subset(clim.tab.fill, Station_ID %in% shortlist[ii]) [1,]
stid <- station$Station_ID[1]  
stnm <- station$Station_Name[1]  
sLat =   station$Lat[1]  
sLon =   station$Lon[1]  
...
monind <- c(12,1:12,1)
Elev <- Elev1
climtab <- subset(clim.tab2, select=c(Lat,Lon,Elev1,Mon,p,t,th,tl))
climtab$stid <- stid; climtab$stnm <- stnm
climtab <- subset(climtab, select=c(stid,stnm,Lat,Lon,Elev1,Mon,p,t,th,tl))
#Humidity ----
...
climtab$RnRa <- climtab$Rn/climtab$Ra
climtab$thtl <- climtab$th-climtab$tl
