...
climtab$Rso <- (0.75+2*10^-5*Elev)*climtab$Ra 
              pmin(climtab$Rso,pmax(0.3*climtab$Rso, 0.16*(climtab$th-climtab$tl)^0.5*climtab$Ra))
...
climtab$I = (pmax(0,climtab$t)/5)^1.514
I <- sum(climtab$I); climtab$I <- NULL
a = 0.49239+1792*10^-5*I-771*10^-7*I^2+675*10^-9*I^3

climtab$e.tw = 16*(10*pmax(climtab$t,0)/I)^a*(climtab$Dl/12)*(climtab$Days/30)
...
climtab$e.pt <- 1.26 * (climtab$delta / (climtab$delta + gamma))*(climtab$Rn-climtab$Gi)/climtab$lambda*climtab$Days #

climtab$e.pm <- (0.408*climtab$delta*(climtab$Rn-climtab$Gi)+gamma*900/(climtab$t+273)*2*(climtab$Vp-climtab$Vpmin))/(climtab$delta+gamma*(1+0.34*2))*climtab$Days #mm asuming windspeed or 2 m/s

climtab$e.hs <- 0.408*0.0023*(climtab$t+17.78)*(climtab$th-climtab$tl)^0.5*climtab$Ra*climtab$Days

climtab$e.tc <- 0.01333 *((23.9001*climtab$Rs)+50)*pmax(climtab$t,0)/(pmax(climtab$t,0)+15)*(1+(50-pmin(50,climtab$RH))/70)*climtab$Days

climtab <- subset(climtab, select= -c(Vp, Vpmax, Vpmin, delta, lambda, Rns, Rnl, Rso, Rs)) 
...
Rs = min(Rso,max(0.3*Rso, 0.175*(th-tl)^0.5*Ra)) # Estimate of normally measured solar radiation Rs/Rso is limited to 0.3-1 and using formula for Hargreaves with constant of 0.16 inland and 0.19 for coastal
